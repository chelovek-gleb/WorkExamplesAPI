...
"""Тест закрепления комнаты"""
@pytest.mark.order(2)
def test_pin_room(check, room_actions):
    
    json_field = 'is_pinned' # Ожидаемое значение ключа в json-ответе
    expected_value = True # Ожидаемое значение поля в json-ответе

    print('\nТест начинается')
    result = room_actions.pin_room() # сохраняем ответ от метода закрепления комнаты
